(function (window, document) {
    ...
    const search = () => {
        ...
        const results = window.searchIndex.search(e, {
            ...
            const doc = document.getElementById("doc").checked
            const page = document.getElementById("page").checked
            ...
            if (doc === false) { not_allowed.push("Document") }
            if (page === false) { not_allowed.push("Page") }
                ...
                tags.map((tag) => {
                    ...
                    switch (tag) {
                        case "World": 
                            ...
                            break
                        case "Document": 
                            tagBadge.textContent = "💬 Documents"
                            break
                        case "Page": 
                            tagBadge.textContent = "📜 Pages"
                            tagBadge.classList.add("badge-accent")
                            break
                        case "Navigation": 
    ...
    fetch("/search-index.json").then((response) =>
        response.json().then((rawIndex) => {
            ...
            document.getElementById("doc").addEventListener("change", search);
            ...
            document.getElementById("page").addEventListener("change", search);
        })
    );
})(window, document);